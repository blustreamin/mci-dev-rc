...
import { doc, getDoc, setDoc, getFirestore, collection, query, where, orderBy, limit, getDocs } from 'firebase/firestore';
...
export const OutputSnapshotStore = {
    ...
    async getLatestOutputSnapshot(categoryId: string, country = 'IN', language = 'en'): Promise<DemandDoc | null> {
        const db = FirestoreClient.getDbSafe();
        if (!db) return null;
        try {
            const colRef = collection(db, 'mci_outputs', country, language);
            // Sort client-side to avoid composite index
            const q = query(colRef, where('category_id', '==', categoryId));
            const snap = await getDocs(q);
            
            if (snap.empty) return null;

            const docs = snap.docs.map(d => d.data() as DemandDoc);
            // Sort descending by computedAt
            docs.sort((a, b) => (b.computedAt || '').localeCompare(a.computedAt || ''));
            
            return docs[0];
        } catch (e) {
            console.error("[OutputSnapshotStore] getLatestOutputSnapshot failed", e);
            return null;
        }
    },

    async readDeterministic(params: { country: string; language: string; docId: string }): Promise<{ ok: boolean; data?: DemandDoc; reason?: string }> {
        const db = FirestoreClient.getDbSafe();
        if (!db) return { ok: false, reason: "DB_INIT_FAIL" };

        const path = `mci_outputs/${params.country}/${params.language}/${params.docId}`;
        try {
            const snap = await getDoc(doc(db, path));
            if (!snap.exists()) return { ok: false, reason: "NOT_FOUND" };
            return { ok: true, data: snap.data() as DemandDoc };
        } catch (e: any) {
            return { ok: false, reason: e.message };
        }
    },
    ...
    async writeDemandDoc(params: {
        ...
        // 1. Write with Sanitization
        // This prevents the "Unsupported field value: undefined" error
        const safePayload = sanitizeForFirestore(params.payload);
        
        await setDoc(doc(db, path), safePayload);
    ...
    async writeDeterministic(params: {
        ...
        try {
            ...
            // Explicit sanitization before write
            const safePayload = sanitizeForFirestore(params.payload);
